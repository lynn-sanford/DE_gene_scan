...
def deseq_add(deseq_results, intersect_regions_filt, raw_tfit, 
    ...
    raw_tfit : list of lists
        object containing raw tfit calls to replace coordinates
    ...
    Returns
    -------
    intersect_regions_deseq : list of lists
        intersect regions with comparison, gene FC and padj appended
    """
    deseq = []
    genes = []
    raw_reg_names = []
    intersect_regions_deseq = []
    ...
    for raw_reg in raw_tfit:
        raw_reg_names.append(raw_reg[3])
    ...
    for region in intersect_regions_filt:
        ...
        raw_region = raw_tfit[raw_reg_names.index(region[3])]
        region[1] = raw_region[1]
        region[2] = raw_region[2]
        intersect_regions_deseq.append(region)
    intersect_regions_deseq.insert(0, ["bidir_chr","bidir_coord1",
                                       "bidir_coord2","bidirID",
                                       "gene_chr","gene_coord1",
                                       "gene_coord2","geneID",
                                       "transcriptID","treatment_comparison",
                                       "log2_fc","padj"])

    return intersect_regions_deseq
...
def main(
    ...
    # 2 : parse TSS regions
    tss_bed = parse_gtf(gtf, (output_dir + "/proc_files"), tss_window)
    ...
    deseq_intersect = deseq_add(deseq_results, filtered_intersect, 
                                raw_tfit_results, significance, thresh)
    if significance:
        deseq_intersect_nofilt = deseq_add(deseq_results, filtered_intersect,
                                           raw_tfit_results, False, thresh)

    # 8 : write out results
    if significance:
        outfile = base_name + "_gene_scan_" + str(scan_window) + "_pfilt.txt"
        outfile2 = base_name + "_gene_scan_" + str(scan_window) + ".txt"
        write_bedfile(
        ...
        write_bedfile(
            "{}/{}".format(output_dir, outfile2),
            deseq_intersect_nofilt,
            12,
        )
    else:
        outfile = base_name + "_gene_scan_" + str(scan_window) + ".txt"
        write_bedfile(
            "{}/{}".format(output_dir, outfile),
            deseq_intersect,
            12,
...
parser.add_argument(
    "-d", "--deseq_res", dest="deseq_results", help="Path to DESeq2 results", metavar="INT",
)
